...
public class CommunityNewsService {
     ...
     * 普通用户查询列表
     */
    public PageBean<CommunityNews> findList(long communityId, int newsType, int noticeType, long userId, String[] tags, int page, int count) {
        List<CommunityNews> list;
        Page p = PageHelper.startPage(page, count);//为此行代码下面的第一行sql查询结果进行分页
        list = todayNewsDao.findList(communityId, newsType, noticeType, "#" + userId + "#", tags);
     ...
     * 管理员查询列表
     */
    public PageBean<CommunityNews> findListByAdmin(long communityId, int newsType, int noticeType, int page, int count) {
        List<CommunityNews> list;
        Page p = PageHelper.startPage(page, count);//为此行代码下面的第一行sql查询结果进行分页
        list = todayNewsDao.findListByadmin(communityId, newsType, noticeType);
