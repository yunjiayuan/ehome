...
//        DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
     ...
     */
    @Override
    public ReturnData findPositionInfo(@PathVariable long userId) {
        if(userId<=0){
            return returnData(StatusCode.CODE_PARAMETER_ERROR.CODE_VALUE,"参数有误",new JSONObject());
        }
        Map<String,Object> positionMap = redisUtils.hmget(Constants.REDIS_KEY_USER_POSITION+userId);
        if(positionMap==null||positionMap.size()<=0){//缓存中不存在
            return returnData(StatusCode.CODE_SUCCESS.CODE_VALUE,StatusCode.CODE_SUCCESS.CODE_DESC,new JSONObject());
        }
        PositionInfo positionInfo = (PositionInfo) CommonUtils.mapToObject(positionMap,PositionInfo.class);
        if(positionInfo==null){//缓存中数据异常
            return returnData(StatusCode.CODE_SUCCESS.CODE_VALUE,StatusCode.CODE_SUCCESS.CODE_DESC,new JSONObject());
        }
        return returnData(StatusCode.CODE_SUCCESS.CODE_VALUE,StatusCode.CODE_SUCCESS.CODE_DESC,positionInfo);
    }
}
